		...
		<div class="card-body">
			...
			<form method="POST">
				...
				<div class="form-group">
					...
					<input type="text" id="name_add" class="form-control" name="name_add" value="{{ old('name_add') }}" placeholder="Nhập vào tên bài viết">
				</div>
				...
				<div class="form-group">
			     	<label for="sel1"><span class="fa fa-tag"></span> <b> Thể loại:</b></label>
			      	<select class="form-control" name="cate_add" id="cate_add_post">
						@foreach($cate as $val)
			        	<option
						@if(session('danger')) 
						@if($val->id == old('cate_add'))
						{{"selected"}}
						@endif
						@endif
				...
				<div class="form-group">
			     	<label for="sel1"><span class="fa fa-paperclip"></span> <b> Chủ đề:</b></label>
			      	<select class="form-control" name="chude_add" id="chude_add_post">
						@foreach($chude as $val)
						<option
						@if(session('danger'))
						@if($val->id == old('chude_add'))
						{{"selected"}}
						@endif
						@endif
						value="{{ $val->id }}">{{ $val->name }}</option>
				...
				<div class="form-group">
					...
					<textarea name="intro_add" id="content-tomtat">{!! old('intro_add') !!}</textarea>
				...
				<div class="form-group">
					...
					<textarea name="full_add" id="content-noidung">{!! old('full_add') !!}</textarea>
					<script type="text/javascript">
						confi = {};
						confi.entities_latin = false;
						confi.language = 'vi';
